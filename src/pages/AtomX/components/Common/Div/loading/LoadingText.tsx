...
import { LoadingDiv, toLineProps, UniversalDivComponentProps } from "../index";
...
export function LoadingTextComponent({
  loadingProps,
  ...rest
}: LoadingTextComponentProps) {
  const { lineCount, lineWidth, lastLineWidth, ...loadingStyledProps } =
    loadingProps;
  const fullLineCount = lineCount - 1;

  const lineProps = toLineProps(
    loadingProps.fontSizePreset ?? rest.fontSizePreset
  );
  const fullLineProps = {
    width: lineWidth ?? "100%",
  ...
  for (let i = 0; i < fullLineCount; i++) {
    lines.push(
      <LoadingDiv key={`text-line-${i + keySeed}`} {...fullLineProps} />
    );
  }
  ...
  lines.push(
    <LoadingDiv
      key={`text-line-${fullLineCount + keySeed}`}
      {...lastLineProps}
    />
  );
